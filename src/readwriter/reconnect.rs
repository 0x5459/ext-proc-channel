...
/// use ext_proc_channel::readwriter::{reconnect::connect as reconnect, pipe::Pipe};
///
/// #[tokio::main]
        ...
        loop {
            let next_state = match self.as_mut().project().state.project() {
                ...
                StateProj::WaitingForRetry(sleep_fut) => {
                    let _ = ready!(sleep_fut.as_mut().poll(cx));
...
mod tests {
    ...
    async fn test_reconnect_read() {
        ...
        let mut r = connect::<RandomDisconnect<_>, _, _>(
            LockedIO::new(expect_data.as_slice()),
    ...
    async fn test_reconnect_write() {
        ...
        let mut w =
            connect::<RandomDisconnect<_>, _, _>(locked.clone(), repeat(Duration::from_millis(10)));
